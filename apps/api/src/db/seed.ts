...
for (const app of seededApplications) {
  ...
  await pool.query('DELETE FROM application_stages WHERE application_id = $1', [app.id]);
  await pool.query('DELETE FROM citizen_actions WHERE application_id = $1', [app.id]);
