...
describe('API', () => { const apps: ReturnType<typeof buildApp>[] = []; const create = () => { const app = buildApp(new MemoryRepository(), new DisabledExplanationService()); apps.push(app); return app; }; afterEach(async () => { await Promise.all(apps.splice(0).map((app) => app.close())); });
  ...
  it('returns the seeded explanation when live AI is disabled', async () => { const response = await create().inject({ method: 'POST', url: '/api/v1/applications/INC-2026-01842/explanation?submissionDate=2026-08-16' }); expect(response.statusCode).toBe(200); expect(response.json().source).toBe('seeded'); });
});
